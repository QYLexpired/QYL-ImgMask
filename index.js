...
class Plugin extends siyuan.Plugin {
    ...
    renderMasksForImg(img, ancestor, maskDataList, getDragMode, onDataChange) {
        ...
        this.getMasksForSrc(maskDataList, img.dataset.src).forEach((data, idx) => {
            ...
            mask.addEventListener('click', e => this.handleMaskClick(e, mask, getDragMode));
            mask.addEventListener('mousedown', e => this.handleMaskMouseDown(e, mask, getDragMode, img, ancestor, maskDataList, () => this.renderMasksForImg(img, ancestor, maskDataList, getDragMode, onDataChange), onDataChange));
    ...
    handleMaskMouseDown(e, mask, getDragMode, img, ancestor, maskDataList, refreshMasks, onDataChange) {
        ...
        let timer = setTimeout(async () => {
            ...
            const masks = this.getMasksForSrc(maskDataList, img.dataset.src);
            masks.splice(idx, 1);
            this.setMasksForSrc(maskDataList, img.dataset.src, masks);
            await this.saveImgMaskData(ancestor.getAttribute('data-node-id'), maskDataList);
            refreshMasks();
            if (masks.length === 0) {
                const showAllButton = img.parentNode.querySelector('.QYLImgMaskShowAll');
                if (showAllButton) {
            ...
            img.parentNode.addEventListener('mousedown', function(e) {
                if (getDragMode()) {
                    ...
                    if (
                        ...
                        this.handleCreateMaskDrag(e, img, ancestor, maskDataList, getDragMode, onDataChange);
                    }
                }
            }.bind(this));
    ...
    handleCreateMaskDrag(e, img, ancestor, maskDataList, getDragMode, onDataChange) {
        ...
        function onMouseUp(ev) {
            ...
            const masks = self.getMasksForSrc(maskDataList, src);
            masks.push({ left, top, width, height });
            self.setMasksForSrc(maskDataList, src, masks);
            self.saveImgMaskData(ancestor.getAttribute('data-node-id'), maskDataList);
            self.renderMasksForImg(img, ancestor, maskDataList, getDragMode, onDataChange);
            if (masks.length === 1) { 
                const existingShowAllButton = img.parentNode.querySelector('.QYLImgMaskShowAll');
                if (!existingShowAllButton) {
                    const showAllSpan = self.createShowAllButton(img, getDragMode);
    ...
    async initImgMask() {
        ...
        document.querySelectorAll('.protyle-wysiwyg').forEach(async wysiwyg => {
            ...
            for (const blockId in blockImgsMap) {
                let maskDataList = await this.loadImgMaskData(blockId);
                blockImgsMap[blockId].forEach(img => {
                    ...
                    this.renderMasksForImg(img, ancestor, maskDataList, () => dragMode.value, () => {});
                    const masksForThisImg = this.getMasksForSrc(maskDataList, img.dataset.src);
                    let showAllSpan = null;
                    if (masksForThisImg.length > 0) {
                        showAllSpan = this.createShowAllButton(img, dragMode);
                    }
                    const self = this; 
                    span.addEventListener('click', function(e) {
                        ...
                        self.renderMasksForImg(img, ancestor, maskDataList, () => dragMode.value, () => {});
                    });
